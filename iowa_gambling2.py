...
class IowaGamblingTaskGUI(QWidget):
    ...
    def on_deck_click(self, deck_index):
        if self.round_num < 100:
            ...
            self.reward = random.choice(self.deck_rewards[deck_index])  # Randomly select a reward/punishment
            self.total_money += self.reward

        self.round_num += 1
        if self.round_num < 100:
            self.update_labels()
        else:
            self.input_window()  # Show the input window after rounds

    def update_labels(self):
        self.money_label.setText(f'Money: ${self.total_money}')
        if self.round_num < len(self.choices):
            self.reward = self.deck_rewards[self.choices[self.round_num]][self.round_num]
            if self.reward > 0:
                self.money_label(f'You won ${self.reward}')
            else:
                self.money_label(f'You lost ${abs(self.reward)}')
        else:
            self.show_result('')
